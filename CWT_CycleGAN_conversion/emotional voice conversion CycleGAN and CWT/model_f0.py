...
from module_f0 import discriminator, generator_gatedcnn
...
class CycleGAN(tf.keras.Model):
    ...
    def __init__(self, num_features, discriminator = discriminator, generator = generator_gatedcnn, mode = 'train', log_dir = './log'):
        ...
        super().__init__()
        self.input_shape_custom = [None, num_features, None] # [batch_size, num_features, num_frames]
    ...
    def build_model(self):
        ...
        self.input_A_real = tf.placeholder(tf.float32, shape = self.input_shape_custom, name = 'input_A_real')
        self.input_B_real = tf.placeholder(tf.float32, shape = self.input_shape_custom, name = 'input_B_real')
        # Placeholders for fake generated samples
        self.input_A_fake = tf.placeholder(tf.float32, shape = self.input_shape_custom, name = 'input_A_fake')
        self.input_B_fake = tf.placeholder(tf.float32, shape = self.input_shape_custom, name = 'input_B_fake')
        # Placeholder for test samples
        self.input_A_test = tf.placeholder(tf.float32, shape = self.input_shape_custom, name = 'input_A_test')
        self.input_B_test = tf.placeholder(tf.float32, shape = self.input_shape_custom, name = 'input_B_test')
    ...
    def optimizer_initializer(self):
        ...
        self.discriminator_optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate = self.discriminator_learning_rate, beta1 = 0.5).minimize(self.discriminator_loss, var_list = self.discriminator_vars)
        self.generator_optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate = self.generator_learning_rate, beta1 = 0.5).minimize(self.generator_loss, var_list = self.generator_vars) 
