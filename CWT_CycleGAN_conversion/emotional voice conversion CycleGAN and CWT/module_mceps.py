...
def instance_norm_layer(inputs, name=None):
    return tf.keras.layers.LayerNormalization(axis=-1, center=True, scale=True, name=name)(inputs)
...
def residual1d_block(
    ...
    h1_norm = instance_norm_layer(inputs = h1, name = name_prefix + 'h1_norm')
    ...
    h1_norm_gates = instance_norm_layer(inputs = h1_gates,  name = name_prefix + 'h1_norm_gates')
    ...
    h2_norm = instance_norm_layer(inputs = h2, name = name_prefix + 'h2_norm')
...
def upsample1d_block(
    ...
    h1_norm = instance_norm_layer(inputs = h1_shuffle, name = name_prefix + 'h1_norm')

    h1_gates = conv1d_layer(inputs = inputs, filters = filters, kernel_size = kernel_size, strides = strides, activation = None, name = name_prefix + 'h1_gates')
    h1_shuffle_gates = pixel_shuffler(inputs = h1_gates, shuffle_size = shuffle_size, name = name_prefix + 'h1_shuffle_gates')
    h1_norm_gates = instance_norm_layer(inputs = h1_shuffle_gates, name = name_prefix + 'h1_norm_gates')
...
def discriminator(inputs, reuse = False, scope_name = 'discriminator'):
    ...
    with tf.variable_scope(scope_name) as scope:
        ...
        # Output
        dense_layer = tf.layers.Dense(units = 1, activation = tf.nn.sigmoid)
        o1 = dense_layer(d3)
